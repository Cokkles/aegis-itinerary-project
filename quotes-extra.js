...
// AEGIS 2.6.3e direct runtime bootstrap.
// This file is referenced directly by the legacy HTML, so it is the recovery anchor.
(()=>{
  if(window.__AEGIS_DIRECT_RUNTIME_263E__) return;
  window.__AEGIS_DIRECT_RUNTIME_263E__=true;
  const RELEASE='2.6.3e';
  const MODULES=[
    'aq1-hotfix.js',
    'aq2-calendar.js',
  ...
  async function boot(){
    // Wait until the base AEGIS core has actually initialized.
    const started=Date.now();
    while(!window.AEGIS?.Core && Date.now()-started<12000) await new Promise(r=>setTimeout(r,100));
    if(!window.AEGIS?.Core){console.error('AEGIS 2.6.3e: base core never initialized');return;}
    try{
      ...
      window.dispatchEvent(new CustomEvent('aegis-runtime-ready',{detail:{release:RELEASE,source:'quotes-extra-direct'}}));
    }catch(err){
      console.error('AEGIS direct runtime bootstrap failed',err);
      const status=document.getElementById('aegisAuthStatus');
      if(status){status.textContent='Runtime initialization failed: '+String(err.message||err);status.classList.add('aegis-auth-error');}
    }
  }

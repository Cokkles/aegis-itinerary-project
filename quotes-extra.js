...
// AEGIS 2.6.3g current feature runtime bootstrap.
// The authoritative AUTH-1 core is loaded normally by index.html as aegis-core.js.
// Do not inject or lock a second Core here.
(()=>{
  if(window.__AEGIS_DIRECT_RUNTIME_263G__)return;
  window.__AEGIS_DIRECT_RUNTIME_263G__=true;
  const RELEASE='2.6.3g';
  ...
  async function boot(){
    ...
    if(!window.AEGIS?.Core){console.error('AEGIS 2.6.3g: core never initialized');return;}
    try{
      ...
      window.dispatchEvent(new CustomEvent('aegis-runtime-ready',{detail:{release:RELEASE,source:'single-core'}}));
    }catch(err){
